 ...
 * 日本語は洗練ゴシック系（システムフォント）を theme.fontJa で直接指定する。
 *
 * 📝 日本語フォントの埋め込みについて: Noto Sans JP 等の CJK を @remotion/google-fonts
 * （CDN）で読むと unicode-range で数百リクエストに分割され、headless レンダのセットアップが
 * タイムアウトする（実測 363 リクエスト）。埋め込みたい場合は CDN ではなく
 * **ローカル woff2 を public/fonts/ に置いて @font-face で読む**こと（rules/local-fonts.md）。
