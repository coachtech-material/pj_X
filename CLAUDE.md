...
- **研修総量**: 最短の時間で最高の満足度に到達することを方針とする（下限は助成金要件の 10 時間、上限の目安は 100 時間）。現行 `OUTLINE.md`（全8 Part・90 Section）の想定学習時間は **約69時間**（概念 Section は手を動かすサブセクション込みで約45分／本）
...
コンテンツの骨子（何を・どの到達点まで教えるか）は、この HOW で定義し、具体的なカリキュラム設計は `OUTLINE.md`（全8 Part・90 Section）が担う。骨子は固定の背骨であり、教材は全受講者共通の一本とする。骨子が決めるのは「何をどの順で学ぶか」であり、各社固有の業務への適用（それを何の題材で実務に活かすか）は、受講者自身の業務を題材にアフターサポートで伴走する。階層構造は 3層（Part > Chapter > Section）。
...
必要条件と土台を、学びやすい順に並べ替えて教材を構成する。前半は手を動かさない概念、後半は手を動かす実践とし、具体と抽象・ハンズオンの有無が混在しないようにする。概念とハンズオンは Chapter 単位で分離し、ハンズオンは後半各 Part の末尾 Chapter に束ねる（混合は導入のセットアップのみ）。

- **前半（概念）**: 1 導入 ／ 2 ビジネス思考（必要条件 A）／ 3 IT基礎・4 AI基礎（土台）
- **後半（実践）**: 5 Claude Code 基礎・6 応用（必要条件 B・C）／ 7 仕組み化（必要条件 E）／ 8 Git・GitHub（必要条件 D）

全8 Part・35 Chapter・90 Section の具体的な構成（各 Section のゴール・種類・前提・参考資料）は `OUTLINE.md` に定義する。CLAUDE.md は骨子（設計思想）を、`OUTLINE.md` は具体的なカリキュラム設計を担う。執筆上の判断（題材の選択・構成のアレンジ・外部調査）は OUTLINE.md の設計に従いつつ、臨機応変に行う。
