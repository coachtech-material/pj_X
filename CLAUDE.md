...
学習順序（Part 構成）はスキル体系とは別の軸として、学習効果が最大になる配置を OUTLINE.md で設計する（ISSUES.md No.13）。冒頭にはセットアップと委任の初体験（すごさと、思い通りにならない壁の両方）を置く入門 Part を想定し、フリーミアム公開範囲の候補とする（ISSUES.md No.4）。職種別題材や自業務への適用といった実践の軸は ISSUES.md No.3 で扱う。
