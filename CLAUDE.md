...
1. **ハーネスまで扱う**: AI 活用技術の進化「プロンプト → コンテキスト → ハーネス」（2026年時点）の 3 階層を一気通貫で教える。成果を分けるのはモデルの性能差ではなく環境設計の差であり、ハーネスまで設計できてはじめて最も質の高いアウトプットを出せる。その知見は目先のテクニックと違って持続し、業界共通の定着失敗への答えにもなる
2. **オーダーメイドとアフターサポート**: 全受講者共通の教材で終わらせず、受講者の業務に合わせて個別化する。MVP でも職種別題材の選択と自業務のマイプロジェクトを標準装備し、その先にオーダーメイド教材とアフターサポートを用意する

## 提供価値（WHAT）

実業務の成果物と、それを出し続けられる力の 2 つを提供する。

- **研修内で、自分の実業務の成果物が完成する**: 職種別題材と自業務のマイプロジェクトで、提案書・分析レポート・業務アプリなどの実物を研修中に作り上げる
- **修了後も、自力で成果物を出し続けられる**: 「AI にうまく聞く」から「AI に仕事を任せ、検証する」へ転換し、思考→仕様化→実行→仕組み化の型を身につける
