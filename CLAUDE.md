...
- **研修総量**: 最短の時間で最高の満足度に到達することを方針とする（下限は助成金要件の 10 時間、上限の目安は 100 時間）。現行 `OUTLINE.md`（全7 Part・55 Section）の想定学習時間は本編（Part1-6）**約38時間**＋役職別 実践題材集（Part7・選択式）で、概算は概念 約45分／ハンズオン 約75分／本。動画は 1 Section 1 本で合計 **約20時間**を上限の目安とする（実値は執筆時に確定）
...
コンテンツの骨子（何を・どの到達点まで教えるか）は、この HOW で定義し、具体的なカリキュラム設計は `OUTLINE.md`（全7 Part・55 Section）が担う。骨子は固定の背骨であり、教材は全受講者共通の一本とする。各社固有の業務への適用（それを何の題材で実務に活かすか）は、Part7「役職別 実践題材集」（選択式・順次拡張）で受講者自身の業務に近い題材を選び、さらにアフターサポートで個別に伴走する。階層構造は 3層（Part > Chapter > Section）。
...
| E-1 | プロンプト／コンテキスト（CLAUDE.md）／ハーネス（Skill・hooks・MCP・サブエージェント・並列）／仕組みの品質・コスト・ガバナンス |

- ハーネスには Skills・hooks・MCP・サブエージェント・並列運用などの Claude Code 拡張機能群を含む（AI 活用技術の 3 階層「プロンプト → コンテキスト → ハーネス」に対応。差別化を参照）
...
- **知識（座学・概念理解）**: ①コンピュータと環境 ②ネットワークと Web ③データ ④アプリケーション ⑤AI・LLM ⑥セキュリティと情報の扱い ⑦プログラミングの基礎概念。IT パスポート〜基本情報技術の粒度で概念を広く扱う（コードを書く・読むことは前提とせず、AI の出力を読み解ける程度）。アプリケーションの基礎は社内ツールの自作・運用に必要な主要概念を含む
- **態度（心構え）**: 当事者意識・知的誠実さ・やり切る・素早いたたき台・時間あたり価値
- セキュリティは「機密を AI に渡さない判断ができる」として、必要条件 1・2（課題を定める／AI に実行させる）の能力にも織り込む

#### 教材構成（学習順序）

必要条件と土台を、最短で Claude Code を業務活用できるようになる体験順に並べる。座学を前半に積み上げるのでなく、早く動かして手応えを得て、必要な概念をその都度差し込む。概念とハンズオンは Chapter 単位で分け（概念がメイン、ハンズオンは各実践 Part の末尾 Chapter に束ねる）、混合は導入のセットアップのみとする。

全7 Part:
1. **導入** ← 動機づけ＋大きなタスクの丸投げで「うまくいかない」を体感（後半の伏線）＋その教訓として必要条件 A（論点・仮説・構造化・完成条件）
2. **IT基礎** ／ 3. **生成AI基礎** ← 土台（動かすのに要る最低限の前提知識）
4. **Claude Code 基礎** ← 必要条件 B・C（操作・調査・検証）＋主要機能（CLAUDE.md・Skills・MCP・サブエージェント・hooks）を知って使う
5. **Claude Code 応用** ← 必要条件 E（ハーネス・SDD・仕組み化・運用）。1-3 の大タスクを仕組み化して再挑戦し成功する伏線回収
6. **Git・GitHub** ← 必要条件 D（残し・共有）
7. **役職別 実践題材集**（選択式・順次拡張）← 必要条件 A〜E を自業務で統合

全7 Part・28 Chapter・55 Section の具体的な構成（各 Section のゴール・種類・前提・参考資料）は `OUTLINE.md` に定義する。CLAUDE.md は骨子（設計思想）を、`OUTLINE.md` は具体的なカリキュラム設計を担う。執筆上の判断（題材の選択・構成のアレンジ・外部調査）は OUTLINE.md の設計に従いつつ、臨機応変に行う。
...
- **技術面の正**: Anthropic 公式日本語ドキュメント（https://code.claude.com/docs/ja/ ）。機能・手順・ベストプラクティスは必ず公式で裏取りする
...
├── OUTLINE.md               # 共通カリキュラムの設計（目次。全8 Part・90 Section）
