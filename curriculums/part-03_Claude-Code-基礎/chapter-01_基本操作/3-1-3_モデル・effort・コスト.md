...
!!! note "補足"
    本文のスラッシュコマンドは、何をどこで切り替えられるかを理解するための例です（読みながら打つ必要はありません）。実際に切り替えてみるのは、セクション末尾の「やってみよう」です（作業の中で使い分けるのは 3-5-1 以降）。ここに書くモデル名や既定値は 2026年6月時点のもので、今後変わります。
...
## やってみよう: モデルと effort で質とコストを見比べる

同じ依頼を、軽い設定と重い設定で実行し、結果の質とコストの違いを自分の目で確かめます。
...
`/model haiku` と打って軽いモデルに切り替え（選択肢に無ければ `/effort low` で代用）、次を頼みます。

```text
新しい「集中タイマーアプリ」のキャッチコピーを5案出してください。
```

結果を見たら、`/usage` と打って、使ったトークン量と推定コストを確認します。

**3. 重い設定で同じことを試す**

`/model opus` に切り替え（または `/effort high`）、まったく同じ依頼をもう一度出します。出てきた5案を、さきほどのものと見比べます。`/usage` で、コストがどう変わったかも確認します。

**4. 何が窓を占めているか見る**

`/context` と打つと、いまのやり取りでコンテキスト（AI が一度に読む範囲。仕組みは 3-2）を何がどれだけ占めているかが分かります。

!!! warning "注意"
    選べるモデルや既定は、プランや時点で変わります（2026年6月時点では Pro の既定は Sonnet）。一覧に無いモデルは選べないので、出てくる選択肢の中で「軽い側」と「重い側」を選べば十分です。`/usage` のドル額は、月額プランでは使用量の目安で、別途請求される額ではありません。

!!! success "確認"
    同じ依頼でも重い設定のほうが凝った案が出やすいこと、`/usage` の数値（コスト）が設定で変わることを観察できれば成功です。自分のふだんの仕事に「軽い側／重い側」のどちらを当てるか、言葉にできるとなおよいです。出力は毎回違ってよく、例と一致する必要はありません。
