...
!!! note "補足"
    本文の SKILL.md の記述例や呼び出し方は、仕組みを理解するための例です（読みながら書く・打つ必要はありません）。実際に手を動かすのは、セクション末尾の「やってみよう」です（公式の skill-creator で本格的なスキルにするのは 3-5-2、仕組みとして固めるのは Part4）。記法・配置は 2026年6月時点のものとして読んでください。
...
ここまでで、前提を渡す CLAUDE.md（3-3-1）と、手順を再利用するスキルを学びました。2つの使い分けを整理します。
...
繰り返し使えそうな手順を1つ、スキルとして作らせ、`/スキル名` で呼び出して動くことを確かめます。
...
文章のトーンを書き分ける手順を、スキルにしてもらいます。

```text
渡した文章を「丁寧」「カジュアル」「絵文字多め」の3つのトーンに書き分ける手順を、tone-rewrite という名前のスキルにして .claude/skills/ に作ってください。
```

`.claude/skills/tone-rewrite/SKILL.md` ができます。中身（`name`・`description` と手順）を開いて、どんな形か眺めてみましょう。

**3. /スキル名 で呼び出す**

`/tone-rewrite` と打ち、続けて適当な一文を渡します（例:「来週の打ち合わせは15時開始です」）。3つのトーンに書き分けた結果が返れば、保存した手順を呼び出して再利用できたことになります。

!!! warning "注意"
    `/tone-rewrite` で呼び出せないときは、スキルの `description`（いつ使うか）が曖昧かもしれません。その様子を Claude Code に伝えて、名前や説明を一緒に見直しましょう。

!!! success "確認"
    `SKILL.md` ができ、`/tone-rewrite` で呼び出して3トーンの書き分けが返れば成功です。今回は手で素朴に作りましたが、公式の skill-creator を使って本格的なスキルにするのは 3-5-2 で扱います。
...
前提を渡す CLAUDE.md（3-3-1）と、繰り返す作業手順を再利用するスキル（本セクション）を押さえました。手順をスキルに切り出せたのと同じように、CLAUDE.md に積み上がった前提も、増えて長くなったら整理できます。次の 3-3-3 では、文体・規約・各業務の進め方を `.claude/rules/` に役割ごとのファイルへ切り出し、CLAUDE.md から `@import` で取り込んで本体を簡潔に保つ方法を扱います。ここまでの CLAUDE.md・スキルとあわせて、Chapter 3-3 の3つの機能がそろいます。
