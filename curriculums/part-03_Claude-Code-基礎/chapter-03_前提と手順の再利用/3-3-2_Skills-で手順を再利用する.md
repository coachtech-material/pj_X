...
!!! note "補足"
    本文の SKILL.md の記述例や呼び出し方は、仕組みを理解するための例です（読みながら書く・打つ必要はありません）。実際に手を動かすのは、セクション末尾の「やってみよう」です（公式の skill-creator で本格的なスキルにするのは Part3 の本づくり、仕組みとして固めるのは Part4）。記法・配置は 2026年6月時点のものとして読んでください。
...
!!! success "確認"
    `SKILL.md` ができ、`/tone-rewrite` で呼び出して3トーンの書き分けが返れば成功です。今回は手で素朴に作りましたが、公式の skill-creator を使って本格的なスキルにするのは Part3 の本づくりで扱います。
...
前提を渡す CLAUDE.md（3-3-1）と、繰り返す作業手順を再利用するスキル（本セクション）を押さえました。手順をスキルに切り出せたのと同じように、CLAUDE.md に積み上がった前提も、増えて長くなったら整理できます。次の 3-3-3 では、文体・規約・各業務の進め方を `.claude/rules/` に役割ごとのファイルへ切り出して、CLAUDE.md 本体を簡潔に保つ方法を扱います。ここまでの CLAUDE.md・スキルとあわせて、Chapter 3-3 の3つの機能がそろいます。
