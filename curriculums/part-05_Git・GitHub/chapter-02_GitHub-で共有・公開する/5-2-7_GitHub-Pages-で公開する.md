...
!!! info "ポイント"
    本研修では、成果物を **Claude Code に HTML/CSS の静的サイトへ変換させて** 公開します。サイトを生成する専用のツールは導入せず、変換そのものを Claude Code に任せます。これも「How は AI に委ねる」進め方の一つで、変換の指示と、公開された見た目の確認に集中できます。
...
5-2-3 で GitHub に上げた成果物のフォルダ（このアークでは入門書の `my-book`）に移動し、そこで Claude Code を起動します。そのうえで、本文（各章の md）を読みやすい静的サイトに変換してもらいます。
