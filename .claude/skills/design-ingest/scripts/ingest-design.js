...
// 画像拡張子 / Section 番号で始まる図ファイルの命名規約
const IMG_EXT = /\.(png|jpe?g|svg)$/i;
const SECTION_PREFIX = /^(\d+-\d+-\d+)-/; // 例: 2-1-1-current-directory.png -> 2-1-1
...
オプション:
  ...
  --section <X-Y-Z>   指定 Section の図だけ取り込む（例: 2-1-4）
...
function main() {
  ...
  if (!zipPath) {
    throw new Error(
      `取り込む zip が見つかりません。Claude Design から書き出した zip を ${opt.downloads} に置くか、--zip <path> で指定してください。`
    );
  }
  ...
  try {
    ...
    if (opt.section) {
      images = images.filter((f) => {
        const m = path.basename(f).match(SECTION_PREFIX);
        return m && m[1] === opt.section;
      });
    }
    ...
    for (const src of images) {
      ...
      const secNum = m ? m[1] : null;
      const sectionFile = secNum ? findSectionFile(opt.curriculums, secNum) : null;
