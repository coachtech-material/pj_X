...
**対象案件**: 第1引数は案件スラッグ `<client>`。対象 Section（`OUTLINE.md`・`curriculums/`）と素材化する概念図（`assets/diagrams/output/`）は `clients/<client>/` 配下を指す（root `CLAUDE.md` の `<client>` 規約）。

Remotion ワークスペース `video/` は全案件共通のエンジン（root 直下、`.claude` と同様。案件ごとに複製しない）。**案件の素材・成果物は `clients/<client>/assets/videos/` に置く**（画像が `clients/<client>/assets/diagrams/` に出るのと同じ構造）:
- **台本（source・コミット対象）**: `clients/<client>/assets/videos/storyboards/<sectionId>.storyboard.json`
- **完成 mp4**: `clients/<client>/assets/videos/output/<sectionId>.mp4`（サイズが大きいので gitignore、配信は GitHub Release）

エンジンは無改修で使い、以降の手順は次のとおり読み替える（中間ファイルの衝突回避に sectionId を `<client>-<sectionId>` へ名前空間化）:
1. 台本の正本（`clients/<client>/assets/videos/storyboards/<sectionId>.storyboard.json`）を `video/data/<client>-<sectionId>.storyboard.json` へ staging（図素材を `video/public/figures/` に cp する既存パターンと同じ）
2. `node scripts/tts-gemini.mjs <client>-<sectionId>`（音声・props は `video/` 内の再生成可能スクラッチ＝gitignore）
3. `npx remotion render src/index.ts SectionVideo clients/<client>/assets/videos/output/<sectionId>.mp4 --props=data/<client>-<sectionId>.props.json`
4. 挿入は `clients/<client>/curriculums/` の Section に、Release URL（`releases/download/videos/<client>-<sectionId>.mp4`）の `<video>` タグで
